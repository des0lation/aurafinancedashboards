...
st.write("Total veBAL is", totalveBAL / 10 ** 18)
st.write("Aura Finance owned veBAL is", auraveBAL / 10 ** 18)
st.write("Aura Finance dominance is", 100 * auraveBAL / totalveBAL)
...
if 'default_value' not in st.session_state:
    st.session_state['default_value'] = 100 * auraveBAL / totalveBAL
else:
    st.session_state.default_value = 100*st.session_state.aura_share
st.session_state['aura_share'] = st.slider("Select projected AURA veBAL %share", min_value=0.0, max_value=100.0,value=st.session_state.default_value,step=0.1, format="%f")/100
aura_revenue = []
